...
router.get("/", auth.optional, async function (req, res, next) {
  ...
  if (favorited) {
    let user = await User.findOne({ username: favorited });
    if (!user) {
      query.favorited = null;
    } else {
      query.favorites = user.id;
    }
  }
  ...
  Article.find(query)
    ...
    .exec((err, articles) => {
      ...
      if (articles.length > 0) {
        ...
        res.status(200).json("no articles to show");
      }
    });
});
...
router.get("/:slug/comments", auth.optional, async function (req, res, next) {
  ...
  try {
    var article = await Article.findOne({ slug: slug });
    var comments = await Comment.find({ article: article.id }).populate(
      "author"
    );
    async function data() {
